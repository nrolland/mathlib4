...
section mysection_for_coend
    ...
    def mycolimit  : (B ⥤ Type u) ⥤ Type (max u₂ u)  := @Functor.ElementsFunctor B _ ⋙ Cat.connectedComponents
...
def comp (P : Dist.{u} A B) (Q: Dist.{u} B C) : Dist.{max u u₂ v₂} A C  :=
  let plug :  (Cᵒᵖ × A) × (Bᵒᵖ × B) ⥤   (B × C)ᵒᵖ × (A × B)  := (prod.inverseAssociator  _ _ _ ) ⋙
     Functor.prod (Prod.swap _ _) (𝟭 _) ⋙ Functor.prod (prod.inverseAssociator _ _ _) (𝟭 _) ⋙
     (prod.associator  _ _ _ ) ⋙ Functor.prod ((prodOpEquiv B).inverse) (𝟭 _)
  let pq : Cᵒᵖ × A ⥤ Bᵒᵖ × B ⥤ Type u := curryObj (plug ⋙ times P Q)
  let fctr : ((Cᵒᵖ × A) ⥤ (Bᵒᵖ × B ⥤ Type u)) ⥤ ((Cᵒᵖ × A) ⥤  Type (max u u₂ v₂)) :=
    (CategoryTheory.whiskeringRight _ _ _ ).obj mycoend
  fctr.obj pq
